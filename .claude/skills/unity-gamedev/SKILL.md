...
name: unity-gamedev
description: |
  Unity game development patterns and Unity MCP tool usage.

  TRIGGERS: Unity, C#, MonoBehaviour, GameObject, prefab, ScriptableObject,
  SerializeField, Rigidbody, Collider, CharacterController, Unity scene,
  PlayMode test, EditMode test, Unity MCP, manage_gameobject, create_script,
  run_tests, manage_scene, manage_prefabs, .unity, .cs file

  USE WHEN: Creating C# scripts, managing GameObjects, working with prefabs,
  scene operations, running Unity tests, configuring components, setting up
  player/enemy hierarchies, or debugging Unity compilation errors.

  TOKEN BUDGET: ~480 tokens (SKILL.md only), ~1500 with references
---
